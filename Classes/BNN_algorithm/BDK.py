...
class BDK(object):
    ...
    def __init__(self, params):
        ...
        self.__dict__.update(BDK.DEFAULTS, **params_dict)

        self.params.BNN_algorithm_master = 'BDK'
        self.BNN_algorithm_master = self.params.BNN_algorithm_master
        self.params.update_all()

        # Esemble of Neural Networks used for prediction (Teacher)
        self.models_T = []
    ...
    def forward_sample(self, input_all_data, epoch = None, batch_idx = None, output_all_additional_data = None, train = 0):
        ...
        # Teacher prediction
        return_model_T_output = self.model_T.forward(x, epoch = epoch, batch_idx = batch_idx, output_additional_data = output_additional_data, train = 1, use_cuda = self.use_cuda)
        ...
        if train:
            ...
            with torch.no_grad():
                return_model_T_output_S_dataset = self.model_T.forward(x_noise, epoch = epoch, batch_idx = batch_idx, output_additional_data = output_additional_data, train = 0, use_cuda = self.use_cuda)
    ...
    def evaluate(self, input_all_data, epoch = None, batch_idx = None, output_all_additional_data = None, train = 0, return_output = 0, use_cuda = None):
        ...
        # Student evaluate
        return_model_S_output = self.model_S.forward(x, epoch = epoch, batch_idx = batch_idx, output_additional_data = output_additional_data, train = 0)
