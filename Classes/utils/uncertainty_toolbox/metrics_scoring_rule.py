...
def nll_gaussian_multi_output(
    ...
    """
    Negative log likelihood for multi-output gaussian.
    ...
    Returns:
        A 1D array of shape (D,) which calculates the negative log likelihood for each output.
    """
    
    # Get number of outputs (D)
    D = y_pred.shape[1]
    
    # Initialize result array
    nll_per_output = np.zeros(D)
    ...
    for d in range(D):
        ...
        # Compute NLL for this output
        residuals = y_pred_d - y_true_d
        nll_list = stats.norm.logpdf(residuals, scale=y_std_d)
        nll_d = -1 * np.sum(nll_list)
        
        # Potentially scale so that sum becomes mean
        if scaled:
            nll_d = nll_d / len(nll_list)
        
        # Store result
        nll_per_output[d] = nll_d
    
    return nll_per_output
