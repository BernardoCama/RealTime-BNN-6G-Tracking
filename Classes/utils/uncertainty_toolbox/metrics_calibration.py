...
def mean_absolute_calibration_error_multi_output(
    y_pred: np.ndarray,
    y_cov: np.ndarray,
    y_true: np.ndarray,
    num_bins: int = 100,
    ...
    """
    Args:
        y_pred: 2D array (N, D) of the predicted means for the held out dataset.
        y_cov: 3D array (N, D, D) of the predicted covariance (covariance) for the held out dataset.
        y_true: 2D array (N, D) of the true labels in the held out dataset.
        num_bins: number of discretizations for the probability space [0, 1].
        vectorized: whether to vectorize computation for observed proportions.
        recal_model: an sklearn isotonic regression model which recalibrates the predictions.
        prop_type: "interval" to measure observed proportions for centered prediction intervals,
    ...
    Returns:
        A 1D array of shape (D,) which calculates the mean absolute calibration error for each output.
    """
    ...
    mace_per_output = np.zeros(D)
    
    for d in range(D):
        y_pred_d = y_pred[:, d]
        y_std_d = np.sqrt(y_cov[:, d, d])  # Assuming diagonal covariance for now
        if np.any(np.isnan(y_std_d)):
            y_std_d = np.zeros_like(y_std_d)+1e-4   
        y_true_d = y_true[:, d]
        
        # Compute MACE for this output
        # start_time = time.time()
        mace_d = mean_absolute_calibration_error(y_pred_d, y_std_d, y_true_d, num_bins, vectorized, recal_model, prop_type)
        # print("mace {} {}]".format(d, str(datetime.timedelta(seconds=time.time() - start_time))))
        
        # Store result
        mace_per_output[d] = mace_d
    
    return mace_per_output

def root_mean_squared_calibration_error_multi_output(
    y_pred: np.ndarray,
    y_cov: np.ndarray,
    y_true: np.ndarray,
    num_bins: int = 100,
    ...
    Returns:
        A 1D array of shape (D,) which calculates the root mean squared calibration error for each output.
    """
    
    D = y_pred.shape[1]
    rmsce_per_output = np.zeros(D)
    ...
    for d in range(D):
        ...
        # Compute RMSCE for this output
        rmsce_d = root_mean_squared_calibration_error(y_pred_d, y_std_d, y_true_d, num_bins, vectorized, recal_model, prop_type)
        
        # Store result
        rmsce_per_output[d] = rmsce_d
    
    return rmsce_per_output


def miscalibration_area_multi_output(
    y_pred: np.ndarray,
    y_cov: np.ndarray,
    y_true: np.ndarray,
    num_bins: int = 100,
    ...
    Returns:
        A 1D array of shape (D,) which calculates the miscalibration area for each output.
    """
    
    D = y_pred.shape[1]
    ma_per_output = np.zeros(D)
    ...
    for d in range(D):
        ...
        # Compute MA for this output
        ma_d = miscalibration_area(y_pred_d, y_std_d, y_true_d, num_bins, vectorized, recal_model, prop_type)
        
        # Store result
        ma_per_output[d] = ma_d
    
    return ma_per_output
